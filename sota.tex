...
%     \item Very few works provide their resources in an open and accessible format (column O)
% \end{itemize}
\begin{table}[]
\footnotesize
\centering
...
An overview of such research projects provides valuable insights into stakeholder requirements and developed solutions within the state of the art, as well as indication of motivation and further work in the future.
...
The approaches model process flows across both - ex-ante and ex-post - phases, though modelling of both within the same approach is less prevalent.
Approaches modelling process flows also do not necessarily model their concepts to match or align with the terminology utilised by the GDPR. 

\subsubsection{Representation of Consent}\label{sota:analysis:consent}
Column headings are as follows - (PD): Personal Data, (Pu): Purpose, (Pr): Processing, (Sh): Data Sharing, (St): Data Storage, (Rp): Recipients, (S): Data Source, (W): Withdrawal of consent, (D): Delegation, (V): Visualisation, (SE): Significant effects of processing, (Ct): Context, (T): Type.
\begin{table}
\footnotesize
\centering
...
\end{tabularx}
\end{table}
1> consent attributes modelled in individual projects
2> does not meet all criteria of consent for GDPR
3> no semantic web vocabularies
...
Column headings are as follows - (M): Method used for evaluation, (Sc): Scope of evaluation, (EA): Ex-Ante, (EP): Ex-post, (MR): Machine-readable outcome, (R): Suggests remedies and recommendations, (LG): Links results to GDPR.
\begin{table}
\footnotesize
\centering
...
RestAssured & \cmark XACML & Process Flows & \cmark & \cmark & \cmark &  &  \\ \hline
...
\end{tabularx}
\end{table}
1> approaches to evaluate
2> no approach for documentation - potentials exist
3> linking information back to GDPR 
...
(1) no machine-readable representation of GDPR exists
(2) no glossary of terms and concepts associated with GDPR exists
(3) no ontology for ex-ante and ex-post phases exists
(4) Lack of authoritative compliance queries
(5) No methodology for recording of compliance assessment as documentation
- no semantics or methods for certification \& seals
- coverage of GDPR articles
...
Where legal compliance is closely tied to functioning of an organisation, requirements of compliance need to be incorporated into relevant activities (commonly called \textit{business processes}) within the organisation.
...
Peer-reviewed publications were the primary source of knowledge regarding approaches, and were identified using scholarly indexing services such as Google Scholar\footnote{\url{https://scholar.google.com/}}, IEEE Explore\footnote{\url{https://ieeexplore.ieee.org/}}, ACM Digital Library\footnote{\url{https://dl.acm.org/}}, Scopus\footnote{\url{https://www.scopus.com/}}, and DBLP\footnote{\url{https://dblp.uni-trier.de/}}.
...
These questions and criteria are used to assess and analyse approaches with the outcome presented in \autoref{sec:sota:analysis} at the end of this chapter.
...
Activities are recorded as a log entry using Policy Log vocabulary \cite{bonatti_special_2018-1} as represented in \autoref{fig:SPECIAL-policy-log-vocabulary} by \texttt{LogEntry}, and are evaluated for compliance using semantic reasoner in both ex-ante and ex-post phases. 
\begin{figure}[htbp]
    \centering
...
PrOnto consists of modules representing (i) documents and data
...
While PrOnto represents a more mature and progressive deliverable of the project's ontology for GDPR, the earlier ontology - referred to as data protection ontology \cite{otake_using_2017} - is relevant for its modelling of concepts and norms based on an draft of GDPR. This ontology uses OWL2 to model concepts of GDPR and was specified to be a work in progress in its publication\footnote{It can be assumed the ontology is no longer being used or developed and was utilised in the development of PrOnto based on subsequent publications of the project \cite{bartolini_legal_2018,bartolini_enhancing_2019}.}.
...
The project uses an ontology to represent the information model as depicted in \autoref{fig:BPR4GDPR-model} \cite{lioudakis_d3.1_2019}. The ontology consists of concepts and relationships regarding data types, roles, operations, and organisation types amongst other contextual concepts.
...
Process mining is performed on knowledge extracted from event logs of
...
A critical analysis of LPL for Article 12-14 of GDPR was carried out \cite{gerl_critical_2018} and presented as requirements identified in articles of GDPR with indication of whether corresponding LPL concepts fulfil stated requirements.
...
The publication makes specific mention of PROV-O \cite{lebo_prov-o_2013} for representing provenance information, supplemented by ontologies for describing additional information about data processing.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\linewidth]{img/Diamantopoulou_meta-model.png}
...
Though the project aims to utilise GDPR as a source of legal requirements and strives to design its framework to meet compliance requirements by both design and default, there is no publicly available information regarding specifics of how GDPR compliance is achieved or represented.
...
\subsection*{Polisis and other approaches based on Usable Privacy Project}
...
\subsection*{Consent Receipt Specification}
...
Following these observations, a more detailed description of analysis is provided for areas identified in \autoref{sec:sota:methodology}. These correspond to investigation of state of the art regarding relevance to research objectives.

\subsection{Representation of GDPR}\label{sota:analysis:representation}
The overview presented in \autoref{table:sota:analysis:overview} shows only three approaches representing structure of GDPR and enabling machine-readable linking of information to specific parts of its text. Of these, two (MIREL and DAPRECO) utilise PrOnto \cite{palmirani_pronto_2018,palmirani_pronto_2018-1} and are therefore consolidated into a single work for purposes of analysis. Therefore, there are only two distinct approaches for representation of GDPR in state of the art. 

\autoref{table:sota:analysis:GDPR} presents an overview of these approaches, along with ELI ontology \cite{ELI_2012} % TODO: citation ELI
used in publication of metadata associated with legislations including GDPR.
The granularity of ELI ontology in representing metadata is limited to specifying information at document level, i.e. it defines GDPR as a legislation but does not provide metadata about its structure or contents. The EU Publication Office, which is in-charge of publishing legislations and maintains ELI ontology, has indicated\footnote{Information communicated via private channels and correspondences, and does not have a public reference.} they are working on updates to ontology - these are represented in the table as `ELI+' for descriptive purposes and completeness of analysis.
\begin{table}[htbp]
\footnotesize
\centering

...
\subsection{Classification of Approaches}\label{sec:sota:models:approaches}
...
The use of structured representation to define legislations for \textit{REO.3} and \textit{REO.7} is possible using standards such as XML and ontologies such as ELI and Akoma Ntoso. However, ELI does not currently offer a way to represent the hierarchical structure of regulations, such as representation of individual articles or clauses, though the European Commission's Publication Office has stated their intention to provide this feature in the future.
...
\subsection{SPECIAL}\label{sec:sota:SPECIAL}\label{sec:sota:}
...
\subsection{MIREL}\label{sec:sota:MIREL}
...
\subsection{DAPRECO}\label{sec:sota:DAPRECO}
...
% TODO: move to semantic web scection
\subsection{ADvoCATE}
...
\subsection{Metamodel for PLA by Diamantopoulou et al.}
...
\subsection{Approaches involving Privacy Policies}
...
% \begin{enumerate}[label={Q{\theenumi}}]
%     \item Quantify how specifically the policy indicates how the first party is obtaining user data 
%     \item Quantify how specifically the policy indicates how the third party is collecting user data 
%     \item Quantify how specifically the policy indicates the type of information accessed by the first party 
%     \item Quantify how specifically the policy indicates the type of information shared with the third party 
%     \item Quantify how specifically the policy indicates how the third party is receiving user information 
%     \item Quantify how specifically the policy covers first party collection purposes relative to all possible purposes in our taxonomy 
%     \item Quantify how specifically the policy covers third party sharing purposes relative to all possible purposes in our taxonomy 
%     \item Quantify how specifically the policy indicates the purpose for data retention
% \end{enumerate}
\begin{figure}[htbp]
    \centering
...
% Another approach regarding consent in the medical domain, the MAGIC project \cite{bialke_magic:_2018}, utilises a modular structure 
...
\subsection{Methodology for Analysis}\label{sota:analysis:methodology}
There are a multitude of approaches utilising technological solutions to represent and evaluate legal compliance, as summarised in \autoref{sec:sota:models}.
The state of the art for this thesis consists of work that adopts and implements these approaches to address GDPR and its compliance.
Analysis of the state of the art is based on assessing these approaches with a particular emphasis on those that utilise semantic web (\autoref{sec:sota:gdpr-semweb}) based on the motivation of the thesis and its research question (\autoref{sec:intro:background}), with the other approaches described in \autoref{sec:sota:gdpr-other}.
Approaches that do not concern or address the GDPR, but are relevant within the domain are also included (\autoref{sec:sota:other}) to provide additional information useful in representing the state of the art.

The surveys presented in \autoref{sec:sota:models} regarding classification of approaches for legal compliance \cite{otto_addressing_2007} and phases of compliance \cite{fellmann_state---art_2014} provide context for the analysis. In addition, the information disseminated by authoritative bodies, presented in \autoref{sec:background:GDPR}, provides an investigative framework to analyse information regarding GDPR compliance.
Based on these, the questions described below are used to categorise and assess the approaches. Following this, each category represented by the question is analysed and discussed in detail to present its state of the art, and for identification of further opportunities and gaps.
\begin{enumerate}
    \item Which aspects of GDPR compliance does the approach target?
    \item How does the approach represent information associated with GDPR and its compliance?
    \item What formalism are used in the approach?
    \item How does the approach represent process flows in terms of phases of compliance?
    \item How does the approach represent information associated with consent?
    \item How does the approach evaluate GDPR compliance?
\end{enumerate}

\subsection{Analysis of existing work}
% For ease of use and understanding, the following labels are used when referring to the work in its entirety as presented in the state of the art instead of individual references.
% \begin{table}[htbp]
%     \centering
%     \begin{tabular}{|c|c|c|}
%         \hline
%         \textbf{Work} & \textbf{Comment} & \textbf{Reference} \\ \hline
%         SPECIAL & SPECIAL H2020 project & \ref{sec:sota:SPECIAL} \\ \hline
%         SPL+SERAMIS & Collaboration of SPECIAL with SERAMIS project & \ref{sec:sota:SPECIAL} \\ \hline
%         SPL+Vos et al. & Collaboration of SPECIAL with Vos et al. & \ref{sec:sota:SPECIAL} \\ \hline
%         SPL+CitySPIN & Collaboration of SPECIAL with CitySPIN project & \ref{sec:sota:SPECIAL} \\ \hline
%         MIREL & MIREL H2020 project & \ref{sec:sota:MIREL} \\ \hline
%         MRL+DAPRECO & Collaboration of MIREL with DAPRECO project & \ref{sec:sota:DAPRECO} \\ \hline
%         BPR4GDPR & BPR4GDPR H2020 project & \ref{sec:sota:BPR4GDPR} \\ \hline
%         Elluri et al. & Work presented by Elluri et al. & \ref{sec:sota:Elluri} \\ \hline
%         Ujcich et al. & Work presented  by Ujcich et al. & \ref{sec:sota:Ujcich} \\ \hline
%         PICS & Personal Information Controll Service & \ref{sec:sota:PICS} \\ \hline
%         Geko and Tjoa & Work presented by Geko and Tjoa &  \\ \hline
%         LPL & & \\ \hline
%         Lodge et al & & \\ \hline
%         Peras & & \\ \hline
%         Tom et al & & \\ \hline
%         Coletti et al & & \\ \hline
%         Corrales et al & & \\ \hline
%         LUCE & & \\ \hline
%         AdvoCATE & & \\ \hline
%         Singh et al & & \\ \hline
%         Sion et al & & \\ \hline
%         privacyTracker & & \\ \hline
%         Spagnuelo et al & & \\ \hline
%         Diamantopulou et al & & \\ \hline
%         Robol et al & & \\ \hline
%         GuideMe & & \\ \hline
%         Basin et al & & \\ \hline
%         RestAssured & & \\ \hline
...
%     \end{tabular}
%     \caption{Labels used to refer to work in the SotA}
%     \label{table:sota:analysis:labels}
% \end{table}

\subsubsection{Overview}
An overview of the state of the art from \autoref{sec:sota:gdpr-semweb} and \autoref{sec:sota:gdpr-other} is presented in \autoref{table:sota:analysis:overview}. The column headings represent the following - (T): type of work where PRJ is research project and RES is research publication, (R): Representation of GDPR, (MG): Modelling of Concepts, (EA): Ex-Ante Compliance, (EP): Ex-Post Compliance, (P): Process Flows, (C): Consent, (Cm): Evaluates Compliance, (Rq): Specifies Requirements, (O): open \& accessible resources, and (Ref) indicating the section where the work is described within the thesis. 
The overview provides a comparison and basis for analysis of approaches, and brings attention to the following:
\begin{itemize}
    \item There are a significant number of research projects addressing or incorporating aspects of GDPR compliance (column T).
    \item There are only three approaches that utilise a representation of GDPR in a machine-readable format (column R)
    \item Most approaches model concepts from the GDPR (column M)
    \item A significant number of approaches model or work with process flows (column P)
    \item A significant number of approaches model or work with metadata about consent (column C)
    \item Evaluation of compliance is carried out by most approaches (column Cm)
    \item A significant number of approaches provide requirements and suggestions for compliance (column Rq)
    \item Most approaches do not provide their resources in an open and accessible format (column O)
\end{itemize}
% \subsubsection{European Research Projects}
The overview also shows a significant number of European research projects addressing or incorporating the GDPR. Given that the GDPR is itself an European law, it is not surprising to have research projects funded by the EU researching applications of technology for its compliance. These projects represent dedicated resources in terms of funding and personnel, as well as collaboration between universities, research institutes, and the industry - which often also includes participation of legal experts and legal organisations.
...
These also provide valuable insights into stakeholder requirements and developed solutions within the state of the art, as well as indication of motivation and further work in the future.
% Table X4 outlines time of operation for the research projects as well as the work presented in this thesis for a perspective on their comparison and analysis.
% TODO: Table table of EU research projects
...
\textbf{Work} & \textbf{T} & \textbf{R} & \textbf{M} & \textbf{EA} & \textbf{EP} & \textbf{P} & \textbf{C} &\textbf{Cm} & \textbf{Rq} & \textbf{O} & \textbf{Ref.} \\ \hline

SPECIAL & PRJ &  & \cmark & \cmark & \cmark & \cmark & \cmark & \cmark &  & \cmark & \ref{sec:sota:SPECIAL} \\ \hline
SPL+SERAMIS & PRJ & \cmark & \cmark &  &  &  &  & \cmark & \cmark & & \ref{sec:sota:SPECIAL}  \\ \hline
SPL+Vos et al & RES &  & \cmark &  &  &  &  & \cmark & \cmark & \cmark & \ref{sec:sota:SPECIAL}  \\ \hline
SPL+CitySPIN & PRJ &  & \cmark & \cmark & \cmark & \cmark & \cmark & \cmark &  &  \cmark & \ref{sec:sota:SPECIAL} \\ \hline
MIREL & PRJ & \cmark & \cmark & \cmark & \cmark & \cmark &  & \cmark & \cmark & & \ref{sec:sota:MIREL} \\ \hline
MRL+DAPRECO & PRJ & \cmark & \cmark & \cmark & \cmark & \cmark &  & \cmark & \cmark & & \ref{sec:sota:DAPRECO}  \\ \hline
BPR4GDPR & PRJ &  &  & \cmark & \cmark & \cmark &  & \cmark & \cmark & & \ref{sec:sota:BPR4GDPR}  \\ \hline
Elluri et al. & RES &  & \cmark &  &  &  &  &  &  & \cmark & \ref{sec:sota:Elluri} \\ \hline
Ujcich et al. & RES &  & \cmark &  & \cmark & \cmark &  &  &  &  & \ref{sec:sota:Ujcich} \\ \hline
PICS & RES &  &  &  & \cmark &  &  &  &  &  & \ref{sec:sota:PICS} \\ \hline
Geko and Tjoa & RES &  & \cmark &  &  &  &  &  &  &  & \ref{sec:sota:Geko} \\ \hline
LPL & RES &  & \cmark &  &  &  &  &  &  &  & \ref{sec:sota:LPL} \\ \hline
Lodge et al & RES &  & \cmark &  & \cmark & \cmark & \cmark & \cmark &  &  & \ref{sec:sota:Lodge} \\ \hline
Peras & RES &  & \cmark &  &  &  & \cmark &  &  &  & \ref{sec:sota:Peras} \\ \hline
Tom et al & RES &  & \cmark & \cmark &  & \cmark &  & \cmark &  &  & \ref{sec:sota:Tom} \\ \hline
Coletti et al & RES &  & \cmark & \cmark &  &  & \cmark &  & \cmark &  & \ref{sec:sota:Coletti} \\ \hline
Corrales et al & RES &  &  &  &  &  &  & \cmark & \cmark &  & \ref{sec:sota:Corrales} \\ \hline
LUCE & RES &  &  &  &  & \cmark &  & \cmark &  &  & \ref{sec:sota:LUCE} \\ \hline
AdvoCATE & RES &  & \cmark &  &  &  & \cmark & \cmark &  &  & \ref{sec:sota:AdvoCATE} \\ \hline
Singh et al & RES &  &  & \cmark &  &  &  &  &  &  & \ref{sec:sota:Singh} \\ \hline
Sion et al & RES &  & \cmark & \cmark &  & \cmark &  & \cmark &  &  & \ref{sec:sota:Sion} \\ \hline
privacyTracker & RES &  & \cmark &  & \cmark & \cmark &  & \cmark & \cmark &  & \ref{sec:sota:privacyTracker} \\ \hline
Spagnuelo et al & RES &  &  &  &  &  &  &  & \cmark &  & \ref{sec:sota:Spagnuelo} \\ \hline
Diamantopulou et al & RES &  & \cmark &  &  &  &  &  &  &  & \ref{sec:sota:Diamantopulou} \\ \hline
Robol et al & RES &  & \cmark & \cmark &  &  &  & \cmark &  &  & \ref{sec:sota:Robol} \\ \hline
GuideMe & RES &  &  &  &  &  &  & \cmark & \cmark &  & \ref{sec:sota:GuideMe} \\ \hline
Basin et al & RES &  &  & \cmark &  & \cmark &  & \cmark &  &  & \ref{sec:sota:Basin} \\ \hline
RestAssured & PRJ &  & \cmark & \cmark & \cmark & \cmark & \cmark & \cmark &  &  & \ref{sec:sota:RestAssured} \\ \hline
Fox et al & RES &  &  &  &  &  &  &  & \cmark &  & \ref{sec:sota:Fox} \\ \hline
DEFeND & PRJ &  & \cmark & \cmark &  &  &  & \cmark &  &  & \ref{sec:sota:DEFeND} \\ \hline
OPERANDO & PRJ &  &  &  &  &  & \cmark & \cmark &  &  & \ref{sec:sota:OPERANDO} \\ \hline
PDP4E & PRJ &  &  &  &  &  &  &  & \cmark &  & \ref{sec:sota:PDP4E} \\ \hline
PoSEID-on & PRJ &  &  &  &  &  & \cmark & \cmark &  &  & \ref{sec:sota:PoSEID-on} \\ \hline
STAR+STAR-II & PRJ &  &  &  &  &  &  &  & \cmark & \cmark & \ref{sec:sota:STAR} \\ \hline
MHMD & PRJ &  &  &  &  &  & \cmark &  &  &  & \ref{sec:sota:MHMD} \\ \hline
PAPAYA & PRJ &  &  &  &  &  &  &  &  &  & \ref{sec:sota:PAPAYA} \\ \hline
SMOOTH & PRJ &  &  &  &  &  &  &  &  &  & \ref{sec:sota:SMOOTH} \\ \hline
SODA & PRJ &  &  &  &  &  &  & \cmark &  &  & \ref{sec:sota:SODA} \\ \hline
DECODE & PRJ &  &  &  &  &  & \cmark & \cmark &  &  & \ref{sec:sota:DECODE} \\ \hline
PRIPARE & PRJ &  &  &  &  &  &  &  & \cmark &  & \ref{sec:sota:PRIPARE} \\ \hline
Galle et al & RES &  & \cmark &  &  &  &  &  &  &  & \ref{sec:sota:Galle} \\ \hline
Polisis & RES &  & \cmark &  &  &  &  &  &  &  & \ref{sec:sota:Polisis} \\ \hline
Linden et al & RES &  & \cmark &  &  &  &  &  &  &  & \ref{sec:sota:Linden} \\ \hline
PrivacyGuide & RES &  & \cmark &  &  &  &  &  &  &  & \ref{sec:sota:PrivacyGuide} \\ \hline
Consent Receipt &  &  &  &  &  &  & \cmark &  &  & \cmark & \ref{sec:sota:CR} \\ \hline

\end{tabularx}
\end{table}
% TODO: delete EA and EP from overview table
...
ELI ontology \cite{} % TODO: citation ELI
...
The modelling of process flows in SotA is further elaborated in \autoref{table:sota:analysis:process-flow} in terms of its use in terms of phase (ex-ante or ex-post), concepts modelled, and representation. The column headings used are as follows - (Repr): representation of process flow, (EA): Ex-ante modelling, (EP): Ex-post modelling, (Pu): Purpose, (Pr): Processing, (DS): Data Sharing, (Rp): Recipients, (St): Data Storage, (Rg): Rights, (LB): Legal Basis.
\begin{table}
\footnotesize
\centering
\caption{Representation of Process Flows in SotA}\label{table:sota:analysis:process-flow}
...
\textbf{Work} & \textbf{PD} & Pu & Pr & Sh & St & Rp & S & W & D & SE & Ct & T \\ \hline
...
Approaches related to privacy policies focus on querying as part of their objective in retrieving relevant information from the text of a privacy policy. In this, the UsablePrivacy project is notable in its use of SPARQL to retrieve information. 

Research projects usually utilise legal experts to provide questions which are then translated into queries using the developed ontologies and technology. Projects such as SPECIAL and MIREL use competency questions in the development of ontologies which are then represented using SPARQL for retrieving information defined using using developed ontologies. An approach uses queries derived from GDPR-compliance checklists provided by ICO to identify coverage of GDPR within privacy policies \cite{linden_privacy_2018}.

From this, it can be summarised that while projects focus on the automation of information representation and compliance evaluation, the utilisation of querying to assist in the compliance process has not seen significant development in the legal compliance domain. Approaches involving privacy policies, by comparison, utilise querying to simplify information retrieval and understanding and demonstrate its potential application towards legal compliance documentation.
...
From the table, most approaches focus on evaluation of process flows followed by fulfilled obligations and given consent. In terms of method or formalism used, semantic web (OWL, RuleML) show significant development, along with utilisation of smart contracts. It can be summarised that approaches evaluate compliance in either ex-ante and ex-post phase, with some approaches covering both phases.
Most approach produce a machine-readable outcome or artefact from the evaluation process that can be persisted for documentation purposes. Some approaches suggest remedial measures to correct or rectify lapses in compliance based on the evaluation. Only three approaches link the evaluation and its outcomes to GDPR. Based on this, it can be concluded that the state of the art provides numerous approaches for evaluating GDPR compliance in ex-ante and ex-post phases.
...
The source of compliance criteria within research projects are the developed requirements and specifications provided by legal experts. The methodologies specified within these approaches do not provide the specific queries or criteria fulfilled by the compliance evaluation nor how such evaluation frameworks can be adapted to assess additional criteria as they arise.
Additionally, these approaches do not demonstrate or indicate how such compliance evaluations can be used to fulfil requirements of certifications and seals as provided by the GDPR. In approaches that produce recommendations or remedies from the evaluation, it is unclear as to how these are recorded against the evaluation and tracked in terms of changes made to the processes across time. 

From this, it can be concluded that the state of the art demonstrates the possibility to automate the evaluation of GDPR compliance and producing machine-readable outcomes. These provide opportunities for utilising the evaluation outcomes in the documentation of ongoing compliance and documenting coverage of compliance GDPR. Furthermore, evaluation in these approaches is carried out under the assumption of validity of existing data and does not consider absence of required information. This also provides an opportunity for assisting with the compliance process by carrying out evaluations that check for validity of information in terms of existence and correctness.

\subsection{Gaps and Opportunities for Further Work}
The following list provides avenues for carrying out research based on opportunities identified from gaps within the state of the art:
\begin{enumerate}
...
\item \textbf{Process flows in ex-ante and ex-post phases:} A cohesive representation of process flows across both ex-ante and ex-post phases that can indicate the relationship between a plan and its execution. Existing vocabularies of PROV-O and P-Plan can be extended with the above glossary to implement this.
\item \textbf{Representation of consent information:} An ontology for representing information associated with consent covering all the requirements of GDPR. This can incorporate the representation of process flows from above to indicate activities.

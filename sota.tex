...
The second section (\autoref{sec:sota:gdpr-semweb}) presents an overview of approaches for GDPR compliance utilising semantic web, the third section (\autoref{sec:sota:gdpr-other}) presents other approaches for GDPR compliance,
and the fourth section (\autoref{sec:sota:other}) presents approaches which do not directly concern the GDPR but present relevant approaches and technology regarding regulatory compliance.
The chapter concludes (\autoref{sec:sota:analysis}) with an analysis of presented approaches and a discussion on research opportunities from identified gaps in the SotA.
...
A survey of approaches for legal compliance within the last 50 years from 1957-2007 for addressing legal requirements \cite{otto_addressing_2007} provides the following categorisation:
\begin{itemize}
...
Based on these, the questions described below are used to categorise and assess the approaches. Following this, each category represented by the question is analysed and discussed in detail to present its state of the art with identification of research opportunities from identified gaps.
\begin{enumerate}
...
Research projects usually utilise legal experts to provide questions which are then translated into competency questions and used in the development of ontologies and technology. Projects such as SPECIAL and MIREL demonstrate use of SPARQL queries derived from competency questions to retrieve information using developed ontologies. However, none of the existing approaches demonstrate an application of such queries to assist in the compliance process by adopting the checklist or questionnaires, such as those provided by data protection offices and the STAR project, to assist and evaluate GDPR compliance preparedness.
...
The source of compliance criteria within research projects are the requirements and specifications provided by legal experts. The methodologies specified within these approaches do not provide the specific queries or criteria fulfilled by the compliance evaluation nor how such evaluation frameworks can be adapted to assess additional criteria as they arise.
Additionally, these approaches do not demonstrate or indicate how such compliance evaluations can be used to assist in the maintenance and demonstration of compliance - such as by ensuring adherence to requirements published by authoritative bodies or evaluation fulfilment towards certifications and seals as provided by the GDPR. In approaches that produce recommendations or remedies from the evaluation, it is unclear as to how these are recorded against the evaluation and tracked in terms of changes made to the processes across time. 

...
The activity is recorded in a log using the Policy Log vocabulary \cite{bonatti_special_2018-1} and is evaluated for compliance by using the reasoner in both ex-ante and ex-post phase. The Policy Log vocabulary, described in D2.7 \cite{kirrane_d2.7_2018}, reuses existing vocabularies such as DCAT and PROV \cite{lebo_prov-o:_2013}, and is used to record an immutable documentation of compliant activities and their legal basis in consent.
...
% TODO: add reference
% TODO: add diagram - if available
The project is applied over the use-case of health data and devices, described in deliverable D1.1, and uses an ontological resource to model the common data ontology - described more in its deliverable D4.2 \cite{teodoro_d4.2-mhmd-ontological-resources.pdf_2018}. The common data ontology consists of modular health data ontologies
DATS core data model figure 2 page 33 showing ontology for data set and its distribution, storage, access, data typeconsent types reflect whether consent is required e.g. synthetic data, fully anonmisedtable 2 page 46 list of parameters - datatype, consents, cloudconsent management interface figure 7, page 41

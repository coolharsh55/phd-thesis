...
\begin{enumerate}
    ...
    \item Link processes based on dependency, order of execution, and composition.
\end{enumerate}

\subsection{Extending PROV-O and P-Plan}
Based on the above stated requirements for representing activities or processes in ex-ante and ex-post phases, the existing semantic web vocabularies of PROV-O \cite{} and P-Plan \cite{} were chosen to be extended with relevant GDPR terminology. A brief overview of PROV-O and P-Plan is provided below in order to understand the underlying concepts and the need to extend them for GPDR.
...
\subsubsection{Creating GDPRov as an extension of PROV-O and P-Plan}
At the time of this undertaking, no other vocabulary was found that represented information about activities associated with GDPR compliance. The decision to extend PROV-O and P-Plan with GDPR concepts was made as both vocabularies contain generic concepts associated with provenance of activities and workflows, which can be used for representing information about GDPR compliance, but will not enable distinguishing between the different types of activities and their relationships in association with compliance.

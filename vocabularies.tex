...
From the above, GDPRov is shown to support the representation and answering of questions (using SPARQL) within the scenario. While the scenario relies on the use of SPECIAL's policy-based vocabulary and the immutable distributed ledger to store and retrieve information, these have not been replicated within the GDPRov representation.
One important conclusion from the above exercise is the representation of given consent - while SPECIAL represents the given consent in the form of a policy, GDPRov does not readily provide a compatible representation of information relevant within the given consent. Instead, it associates the given consent as a legal basis with the purposes (specified as processes) that rely on it. 
The same scenario is also used to demonstrate application of GConsent in \autoref{sec:gconsent:use-case:SPECIAL} to provide a consent-centric management of information, and to showcase its differences from GDPRov.

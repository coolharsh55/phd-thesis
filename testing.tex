...
The constraint it linked to the Article 4-11 of GDPR using the property \texttt{linkToGDPR} mentioned above, and provides a human readable message when the constraint fails using the SHACL property \texttt{sh:message}.
\begin{listing}[htbp]
\begin{minted}[
    frame=single,
    framesep=5mm,
...
The privacy policy also described GDPR rights regarding right to access, right to rectify, right to restrict processing, right to deletion, right to data portability, and provided a link\footnote{NOTE: In reality, we could not access the rights information page, as the notice on the webpage specified - "No cookie found: We are unable to find a Quantcast measurement cookie on your browser and therefore will be unable to identify any data in our systems or process any request for data access or deletion."} for contact and more information. This link was used as the IRI for activities associated with these rights.
...
\begin{table}
\centering
\caption{Report showing constraints, validation results, and link to GDPR}
\label{table:shacl:sparql-report}
\begin{tabular}{|l|c|c|c|c|}
\hline
\rowcolor{lightgray} Name & Type & GDPR & Result & Node \\ \hline
...
Withdraw Consent Information & M & A7-3 & P &  \\ \hline
\end{tabular}
\end{table}
...
With this, the conclusion that re-usability of resources is limited to cases where the developed ontologies are used is true, but is not absolute.
